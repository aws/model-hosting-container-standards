...
def _resolve_transforms(handler_type: str, request_shape: Dict[str, Any], response_shape: Dict[str, Any]):
    ...
    logger.debug(f"Creating transformer instance: {getattr(_transformer_cls, '__name__', str(_transformer_cls))}")
...
def create_transform_decorator(handler_type: str):
    ...
    def decorator_with_params(request_shape: Dict[str, Any] = {}, response_shape: Dict[str, Any] = {}):
        ...
        def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            ...
            if not request_shape and not response_shape:
                logger.info(f"No transform shapes defined, using passthrough")
                handler_registry.set_handler(f"{handler_type}", func)
                logger.info(f"[{handler_type.upper()}] Registered transform handler for {func.__name__}")
                return func
            
            # Resolve transforms as needed
            transformer = _resolve_transforms(handler_type, request_shape, response_shape)

            # Create wrapped function that applies transforms
            logger.info(f"[{handler_type.upper()}] Transform decorator applied to: {func.__name__}")
            ...
            async def decorated_func(raw_request: Request):
                ...
                transform_request_output = await transformer.transform_request(raw_request)
