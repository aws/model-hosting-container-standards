...
class UnloadLoraApiTransform(BaseLoRAApiTransform2):
    def __init__(
        self,
        original_function,
    ...
    def _init_validate_sagemaker_params(self, sagemaker_param):
        if sagemaker_param not in SageMakerUnloadAdapterRequest.model_fields.keys():
            raise ValueError(
                f"Invalid sagemaker_param: {sagemaker_param} for UnloadLoraApiTransform. "
                f"Allowed value(s): {SageMakerUnloadAdapterRequest.model_fields.keys()}"
            )
