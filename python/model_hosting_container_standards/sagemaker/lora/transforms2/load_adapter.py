...
from ..base_lora_api_transform2 import BaseLoRAApiTransform2, LoRARequestBaseModel
...
class LoadLoraApiTransform(BaseLoRAApiTransform2):
    def __init__(
        self,
        original_function,
    ...
    def _init_validate_sagemaker_params(self, sagemaker_param):
        if sagemaker_param not in SageMakerLoadAdapterRequest.model_fields.keys():
            raise ValueError(
                f"Invalid sagemaker_param: {sagemaker_param} for LoadLoraApiTransform. "
                f"Allowed value(s): {SageMakerLoadAdapterRequest.model_fields.keys()}"
            )
    ...
    async def validate_request(
        ...
        try:
            validated_request = SageMakerLoadAdapterRequest.model_validate(
                raw_query_params
            )
            logger.info(
                f"Load adapter request validated successfully for adapter: {validated_request.name}"
            )
            logger.debug(
                f"Validated parameters - name: {validated_request.name}, src: {validated_request.src}, "
    ...
    def _generate_successful_response_content(
        ...
        logger.info(
            f"Generated successful load response for adapter: {adapter_alias or adapter_name}"
        )
