...
class UnloadLoraApiTransform(BaseLoRAApiTransform):
    ...
    async def validate_request(self, raw_request: Request) -> SageMakerUnloadAdapterRequest:
        ...
        try:
            ...
            logger.info(f"Unload adapter request validated successfully for adapter: {adapter_name}")
            ...
            raise HTTPException(
                ...
                detail=f"Malformed request: Unable to extract adapter name from request path",
            )
    ...
    def _generate_successful_response_content(self, raw_response: Response, transform_request_output: BaseLoRATransformRequestOutput) -> str:
        ...
        logger.info(f"Generated successful unload response for adapter: {adapter_alias or adapter_name}")
