...
class BaseLoRAApiTransform(ABC):
    def __init__(
        ...
        logger.debug(f"Initialized {self.__class__.__name__} with paths: {engine_request_paths}")
    ...
    def _extract_additional_fields(self, validated_request: LoRARequestBaseModel, raw_request: Request) -> LoRAAdditionalFields:
        ...
        logger.debug(f"Extracted adapter fields - name: {adapter_name}, alias: {adapter_alias}")
    ...
    def _transform_sagemaker_request_to_engine(self, transformed_request: TransformedRequest, sagemaker_request_dict: Dict[str, Any]) -> TransformedRequest:
        logger.debug(f"Transforming SageMaker request to engine format. Input: {sagemaker_request_dict}")
        
        for sagemaker_param, engine_path in self.engine_request_paths.items():
            if engine_path is not None:
                value = sagemaker_request_dict.get(sagemaker_param)
                logger.debug(f"Mapping {sagemaker_param}={value} to engine path: {engine_path}")
    ...
    def transform_request(self, validated_request: LoRARequestBaseModel, raw_request: Request) -> BaseLoRATransformRequestOutput:
        logger.debug(f"Starting request transformation for adapter: {validated_request.name}")
    ...
    async def call(self, transform_request_output: BaseLoRATransformRequestOutput, func: Optional[Callable] = None, request_model_cls: Optional[BaseModel] = None):
        ...
        adapter_name = transform_request_output.additional_fields.get("adapter_name", "unknown")
        ...
        if request_model_cls is not None:
            try:
                ...
                logger.debug(f"Validating request body with model: {request_model_cls.__name__}")
                ...
                logger.error(f"Request validation failed for adapter {adapter_name}: {e}")
                raise HTTPException(
                    status_code=HTTPStatus.FAILED_DEPENDENCY.value, detail=e.json(include_url=False)
                )
        else:
            logger.debug("No request model validation required, calling function directly")
    ...
    def transform_response(self, raw_response: Response, transform_request_output: BaseLoRATransformRequestOutput):
        ...
        if hasattr(raw_response, "status_code"):
            ...
            logger.debug(f"Processing response for adapter {adapter_name} with status code: {status_code}")
            
            if status_code == HTTPStatus.OK.value:
                logger.info(f"Successfully processed request for adapter: {adapter_name}")
                return Response(
                    status_code=HTTPStatus.OK.value,
                    content=self._generate_successful_response_content(raw_response, transform_request_output),
                )
            else:
                logger.warning(f"Non-OK response for adapter {adapter_name}: {status_code}")
                return raw_response
        else:
            logger.debug(f"Response has no status_code attribute for adapter: {adapter_name}")
            return raw_response
    ...
    async def transform(self, raw_request):
        ...
        try:
            ...
            logger.debug(f"Request validation successful for adapter: {validated_request.name}")
            ...
            final_response = self.transform_response(raw_response, transform_request_output)
