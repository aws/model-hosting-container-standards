"""
Integration tests for SageMaker LoRA functionality using v2 decorators.
Based off test_sagemaker_lora_integration.py
"""
...
class BaseLoRAIntegrationTest:
    ...
    def setup_handlers(self):
        ...
        @sagemaker_standards.register_load_adapter_handler_v2(
        ...
        @sagemaker_standards.register_unload_adapter_handler_v2(
            engine_request_lora_name_path="lora_name",
        ...
        @sagemaker_standards.inject_adapter_id_v2("model")
...
class TestLoRARouterRedirection(BaseLoRAIntegrationTest):
    ...
    def test_register_adapter_route_mounted(self):
        ...
        # Re-setup handlers for this parametrized test
        handler_registry.clear()
        self.setup_handlers()
        sagemaker_standards.bootstrap(self.app)
        ...
        url = f"/adapters?name={lora_name}&src={lora_path}"
    ...
    def test_unregister_adapter_route_mounted(self):
        ...
        self.client.post("/adapters?name=test-adapter&src=s3://bucket/adapter")
    ...
    def test_inject_adapter_id_v2_no_route(self):
        """Test that inject_adapter_id_v2 does not create its own route.

        inject_adapter_id_v2 is a transform-only decorator that modifies requests
...
class TestLoRARequestResponseTransformation(BaseLoRAIntegrationTest):
    ...
    def test_unregister_adapter_path_param_extraction(self):
        ...
        """
        self.client.post("/adapters?name=my-adapter-name&src=s3://bucket/adapter")
    ...
    def test_inject_adapter_id_v2_header_extraction(self):
        """Test that inject_adapter_id_v2 extracts adapter ID from SageMaker header.
    ...
    def test_inject_adapter_id_v2_nested_jmespath_existing_body(self):
        ...
        @sagemaker_standards.inject_adapter_id_v2("body.model.lora_name")
        async def invocations(request: Request):
            body_bytes = await request.body()
            import json
    ...
    def test_nested_jmespath_transformations(self):
        ...
        response = self.client.post(f"/adapters?name={lora_name}&src={lora_path}")
...
class TestLoRAErrorCases(BaseLoRAIntegrationTest):
    ...
    def test_inject_adapter_id_v2_invalid_adapter_path(self):
        """Test that passing an invalid adapter_path with inject_adapter_id_v2 raises error"""
        handler_registry.clear()

        with pytest.raises(ValueError):

            @sagemaker_standards.inject_adapter_id_v2("")
        ...
        with pytest.raises(ValueError):
            ...
            @sagemaker_standards.inject_adapter_id_v2(1234)
            async def invocations_nonstr(request: Request):
                pass

    def test_inject_adapter_id_v2_valid_path(self):
        """Test that inject_adapter_id_v2 accepts valid adapter paths."""
        # Test that the function works correctly with a simple path
        decorator = sagemaker_standards.inject_adapter_id_v2("model")
        assert callable(decorator)

        # Test that it works with nested paths too
        decorator_nested = sagemaker_standards.inject_adapter_id_v2(
...
class TestLoRAEndToEndFlow(BaseLoRAIntegrationTest):
    ...
    def test_full_adapter_lifecycle(self):
        ...
        register_response = self.client.post(
            f"/adapters?name={lora_name}&src={lora_path}"
        )
    ...
    def test_multiple_adapters(self):
        ...
        url_a = "/adapters?name=adapter-a&src=s3://a"
        self.client.post(url_a)

        url_b = "/adapters?name=adapter-b&src=s3://b"
        self.client.post(url_b)

        url_c = "/adapters?name=adapter-c&src=s3://c"
        self.client.post(url_c)
